...
import PlanExpiredPopup from "./PlanExpiredPopup";
...
const MealSection = ({ title, items, onAddPress }) => (
    <View style={styles.mealSection}>
        ...
        {items.map((item, index) => (
            <MealItem key={index} item={item} mealType={title} />
        ))}
    </View>
);
...
const Diary = ({ navigation }) => {
    ...
    console.log("Diary user", user.plan);
    ...
    useEffect(() => {
        if (user && user.plan?.endDate) {
            ...
            const daysSinceExpiration =
                (new Date() - expirationDate) / (1000 * 60 * 60 * 24);
