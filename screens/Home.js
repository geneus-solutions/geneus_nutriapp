...
const Home = ({ navigation }) => {
    ...
    const checkPlanStatus = (plan) => {
        ...
        const expirationDate = new Date(plan.endDate);
        const currentDate = new Date();

        const isExpired = currentDate > expirationDate;

        // Calculate how many days have passed since expiration
        const diffTime = currentDate - expirationDate;
        const diffDays = Math.floor(diffTime / (1000 * 60 * 60 * 24)); // ms to days

        const expiredMoreThan21Days = isExpired && diffDays > 21;

        console.log("Plan check:", {
            planName,
            expirationDate,
            isExpired,
            expiredMoreThan21Days,
        });

        return (
            !expiredMoreThan21Days &&
            (planName === "free trial" || planName === "premium plan")
        );
    };
